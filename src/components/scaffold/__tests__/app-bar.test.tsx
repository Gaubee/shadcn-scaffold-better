...
describe("AppBar", () => {
  ...
  it("renders with leading icon", () => {
    render(<AppBar leading={<Menu data-testid="menu-icon" />} title="Test App" />);
  ...
  it("applies correct elevation classes", () => {
    const { container, rerender } = render(<AppBar title="Test" elevation={0} />);
  ...
  it("handles collapsible mode", async () => {
    const { container } = render(<AppBar title="Test" collapsible expandedHeight={80} collapsedHeight={56} />);
  ...
  it("applies custom className", () => {
    const { container } = render(<AppBar title="Test" className="custom-class" />);
    expect(container.querySelector("header")).toHaveClass("custom-class");
  });

  it("handles different position types", () => {
    const { container, rerender } = render(<AppBar title="Test" position="fixed" />);
